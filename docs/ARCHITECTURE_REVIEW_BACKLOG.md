...
| MCP-2 | P1 | **done** | `17598c7` — `bearer_flag` is codex's built-in default; `mcp_proxy.py` (272 lines) + all proxy threading deleted. Live-confirmed end-to-end: codex connected to `/mcp` via `Authorization: Bearer` (no proxy) and posted a `chat_send` the server authenticated (`CallToolRequest` 200s). 228 tests green; `grep 'McpIdentityProxy|mcp_proxy|_start_identity_proxy|needs_proxy|proxy_url|proxy_flag' *.py` → 0 |
...
| FE-6 | P1 | **done (code + runtime-verified)** | `ef00925` — `messagesById` Map model; copy/reply/recolor/pins/rename/convert-to-job read the model, `dataset.rawText` gone. Live headless probe: all read paths correct, reply-quote colour equivalence confirmed, 0 exceptions |
| NEW-FE-chatjs-split | P3 | **done (code + runtime-verified)** | Tier-A `0bb67aa`/`4e74327`/`9c92cdd` (sounds/version-pill/help leaves). Tier-B `0a34a4b`/`e989cc1`/`2b3de87` — `_agentRoles`→Store, then settings.js (234 lines) + naming-lightbox.js (356) extracted as feature panels; the roles cluster (ROLE_PRESETS/_setRole/showBubbleRolePicker/fetchRoles) was kept in chat.js as status-adjacent (deviation from the "2 clean files" framing, same kind as FE-2's renderMarkdown/agentHats). chat.js 4254→2971. Static net: per-file `node --check` + 16-script concat check + grep (0 moved defs left in chat.js). **Runtime (RM-6): `showPillPopover` builds (role chips + swatches), settings open/save/close, and the agent-flow probe (status pills, `showBubbleRolePicker`, `agent_renamed` DOM+model sync, pending-name popover) all pass with 0 exceptions** |
...
| MCP-2 | **Done** | codex default→bearer; mcp_proxy.py deleted (272 lines); live codex confirmed end-to-end |
...
| FE-6 | **Done (code + runtime-verified)** | `ef00925` messagesById model; live headless probe passed |
| NEW-FE-chatjs-split | **Done (code); RM-6 verify** | Tier-A 3 leaves + Tier-B 2 panels (settings.js/naming-lightbox.js); chat.js 4254→2971; runtime verify in RM-6 |
...
#### MCP-2 — delete `mcp_proxy.py`, fold codex into direct bearer inject (P1, DONE — `17598c7`)
...
- **Progress — execution note:** the proxy-free `bearer_flag` inject mode is implemented in `mcp_inject.py` (emits `-c mcp_servers.agentchattr.url="<real-url>"` + `-c mcp_servers.agentchattr.bearer_token_env_var="AGENTCHATTR_TOKEN"`, token in `inject_env`, never in argv) and unit-tested (`test_wrapper_mcp_config.BearerFlagInjectTests`). **Now done (`17598c7`):** `_BUILTIN_DEFAULTS['codex']` switched to `bearer_flag`, the `proxy_flag` mode + `proxy_url` threading + `mcp_proxy.py` (272 lines) all removed, and a live codex session confirmed it end-to-end — codex registered, connected to `/mcp` with `Authorization: Bearer` (no proxy), and on `@codex` posted a `chat_send` the server authenticated by token (multiple `CallToolRequest` 200s). 228 tests green; `grep` of every proxy symbol across `*.py` → 0.
...
#### NEW-MCP-2 — MCP read contract serialized in 3 divergent inline shapes (P3, done) · NEW this audit
...
#### FE-6 — hold a messages model; stop using the DOM as the source of truth (P1, DONE — code + runtime-verified `ef00925`)
...
**Status — DONE (`ef00925`), implemented AND runtime-verified autonomously.** The original plan gated this to the real-machine phase because the read-cutover changes how values are *derived* and several model-derived values must equal the previously-scraped rendered values — equivalence only runtime can confirm. That gate was lifted once it was established the UI can be runtime-verified without the user (headless Chrome via the chrome-devtools-mcp CLI driving the `.venv run.py` server — see memory `project_runtime_verification`). The value-equivalence was first proven empirically (a sent message's rendered `.msg-sender` colour equals `getColor(sender)`; the timestamp is the server-preformatted `msg.time`; copy text equals `msg.text`), then the cutover was implemented and a live headless probe confirmed every read path: model populated on send, copy text, `startReply`, the sent reply's reply-quote (sender **and** colour), pins (sender + text), and `recolorMessages` — **0 exceptions**. A follow-up sweep confirmed `showPillPopover`/settings/channels/render-helpers/pins all run clean. `messagesById` (Map keyed by `String(msg.id)`) is declared in chat.js; `window.getMessageById` is the accessor jobs.js's convert-to-job uses. `dataset.rawText` is gone (grep → 0 in chat.js) and `.msg-sender`.textContent/.style reads → 0.

**Scrape sites to repoint (current chat.js lines):**
- `_renderChat` reply-quote block — **459-462** (`parentEl.querySelector('.msg-sender').textContent`, `parentEl.dataset.rawText`, `parentEl.querySelector('.msg-sender').style.color`).
- `el.dataset.rawText = msg.text` write — **473** (the canonical per-node text store the model replaces as source of truth).
- `recolorMessages` — **585-606** (`.msg-sender` query 588; re-render `.msg-text` from `dataset.rawText` 600-602).
- `copyMessage` — **1610-1633** (`const markdown = el.dataset.rawText || msgText?.innerText || ''` at 1616).
- `startReply` — **1635-1660** (`.msg-sender`.textContent 1639; `dataset.rawText || .msg-text.textContent` 1640).
- `renderTodosPanel` (pins/todos) — **1957-1995** (scrapes `.msg-time`/`.msg-sender`/`.msg-text`/`.msg-sender`.style.color at 1978-1981).
- `agent_renamed` WS handler — **2740+** (`.msg-sender`.textContent === old_name → rename, 2753-2754).
- message-edit WS handler — **~2908-2930** (`querySelector('.message[data-id=...]')` 2913; `newEl.dataset.id == updatedMsg.id` 2924).
- `clear` WS handler — **2933+** (`el.dataset.id` / `el.dataset.channel` 2943).
- (related, R cluster) `_syncBubbleRolePills` — **903** (`.msg-sender`.textContent scrape; folds in once the model carries sender).

- **Approach (方案):** `const messages = new Map()` keyed by `msg.id`, value = the raw `msg` object. `appendMessage` (288) does `messages.set(msg.id, msg)` after building the node; the `clear` handler deletes the matching-channel entries; `agent_renamed` rewrites the `sender` field of affected entries; the edit handler rewrites `text`. Reads repoint: `copyMessage`→`messages.get(id).text`; `recolorMessages`→iterate the model; `startReply`/reply-quote→`messages.get(parentId)` and **recompute** the accent via `getColor(sender)`; `renderTodosPanel`→`messages.get(id)` with the existing time formatter + `getColor`.
- **Fix scope (修正範圍):** `chat.js` only; ~10 read sites + `appendMessage`/`_renderChat` + 3 WS handlers (rename/edit/clear). High — root cause behind FE-2's `appendMessage` blocker and the recolor/rename DOM walks.
- **Value-equivalence risk flags (why runtime-gated, verify at RM-6):** (1) reply/pins **accent colour** is currently read from the rendered `.msg-sender`.style.color; the model path recomputes via `getColor(sender)` and must produce the identical colour (override → config → fallback resolution). (2) pins **timestamp** is currently the rendered `.msg-time`.textContent; the model path formats `msg.timestamp` and must match the displayed format exactly. (3) `copyMessage`'s `|| innerText` fallback must be preserved for renderer kinds that have no `msg.text`.
- **Completion criteria (達成條件):** a `Map<id,msg>` owns message data AND `grep 'dataset.rawText'` reads + `querySelector('.msg-sender').textContent` reads in chat.js → 0 (reads come from the model) AND reply/recolor/rename/copy/pins operate on the model, not scraped nodes AND the three value-equivalence flags above confirmed identical in a live session.

#### NEW-FE-chatjs-split — finish the monolith breakup (P3, done — code; RM-6 runtime verify pending) · NEW this audit
...
**Status — Tier-A DONE (code), `0bb67aa`/`4e74327`/`9c92cdd`:** three genuinely-self-contained leaves sliced out (script-based line-slice): `sounds.js` (sound engine, 91 lines — localStorage prefs + `Store.get('agentConfig')` only), `version-pill.js` (update pill, 32 lines — api.js + DOM), `help.js` (the **full** help-tour cluster, 524 lines — DOM/localStorage + its own `_help*` state, **zero** Store/app-state refs, verified by grep). The doc's "~80 line" help estimate was wrong: the cluster (toggleHelp → _helpCardDefs → _openHelpAnchored/Stacked → openHelp/closeHelp/initHelpTour + state) is contiguous and self-contained, so it moved as one. chat.js: **4254 → 3536 lines**. Each leaf's `<script>` is loaded after store.js; callers resolve via classic-script shared global scope + explicit window exports. `node --check` × 13 + grep (each moved fn defined once in its leaf, 0 in chat.js). **Tier-B (naming-lightbox + settings) — DONE (code), `0a34a4b`/`e989cc1`/`2b3de87`:** the two feature panels were extracted with the channels.js/rules-panel.js pattern, in three commits:
1. `0a34a4b` — prep: `_agentRoles` (`const {}` in chat.js, read by the core `_renderChat` renderer + `updateStatus`/`fetchRoles` status handlers + both panels) promoted to a Store key `agentRoles`, mutate-in-place exactly like FE-2's `colorOverrides`. This is what lets `showPillPopover` read roles across the file boundary. 8 refs migrated, seed added, def removed (grep `_agentRoles` chat.js → 0).
2. `e989cc1` — `settings.js` (234 lines): `pendingChannelSwitch` (+ its `window._setPendingChannelSwitch` setter, moved off chat.js's shim block so the var and its mutator are co-located here; channels.js still writes it via that window setter), `applySettings`, `toggleSettings`, `clearChat`/`_clearClearChatConfirm`/`_clearChatOutsideClick`, `saveSettings`, `setupSettingsKeys`. Byte-exact slice; calls channels.js's `switchChannel`/`filterMessagesByChannel`/`renderChannelTabs` and reads `ws`/`window.activeChannel` via the shared global scope; window-exports the 4 entry points chat.js/index.html drive (applySettings via the WS handler, setupSettingsKeys via init, toggleSettings/clearChat via inline onclick).
3. `2b3de87` — `naming-lightbox.js` (356 lines): `_pendingNameQueue`/`_nameModalActive`, `_showNextPendingName`, `showAgentNameModal` (note: 0 callers — pre-existing dead code superseded by the pill popover; **moved as-is, not deleted** per the dead-code rule), `_closeAgentNameModal`, `showPillPopover`. Calls back into chat.js's roles cluster (`_setRole`/`_deleteCustomRole`/`ROLE_PRESETS`) and recolor helpers (`recolorMessages`/`buildMentionToggles`), reads `Store.get('agentRoles')` and `BRAND_AVATARS`/`USER_AVATAR` (agentview.js); window-exports `showPillPopover` (buildStatusPills), `_showNextPendingName`/`_pendingNameQueue` (the WS name-prompt handler), `_closeAgentNameModal` (Escape key).

**Deviation from the naive "2 clean files":** the **roles cluster** (`ROLE_PRESETS`, `showBubbleRolePicker`, `_syncBubbleRolePills`, `_setRole`/`_addCustomRole`/`_deleteCustomRole`, `fetchRoles`, `_ROLE_EMOJI`) was deliberately **kept in chat.js**, not split across the two panels. It is status-adjacent — `_agentRoles` is read by the core message renderer, `showBubbleRolePicker` is invoked from a message-bubble onclick, and `_syncBubbleRolePills`/`fetchRoles` are driven by the WS status handlers — so moving it into a UI panel would push the status subsystem cross-file for no benefit. Same kind of judged deviation as FE-2's `renderMarkdown`/`agentHats`. chat.js: **4254 → 2971 lines** (Tier-A+B combined −1283).

**Static verification net (runtime verify deferred to RM-6, per §5 / TaskList):** per-file `node --check` (×3 new + chat.js) + a 16-script concatenation `node --check` in index.html load order (catches any cross-script top-level `const`/`let`/`class` redeclaration — the one real load-time failure mode for shared-global-scope panels; passed) + grep-completeness (each moved symbol defined once in its new file, 0 definitions left in chat.js; the surviving call sites in chat.js resolve to the panels' globals). The remaining risk — a bare cross-module reference failing at runtime — is exactly the feature-panel concern that gates this to RM-6.
...
- **Completion criteria (達成條件):** Tier A: sounds, version-pill, help-tour each defined in their own module with `grep <moved-fn-names> chat.js` → 0 AND the app loads with no missing-global `ReferenceError`. **Met (Tier A `9c92cdd`, Tier B `2b3de87`):** all five panels (sounds/version-pill/help + settings/naming-lightbox) extracted; `grep` of every moved symbol → 0 defs in chat.js; concat `node --check` clean. chat.js landed at **2971** lines, not the original `< ~2500` aspiration — that flat target was explicitly dropped as the gate (FE-3 was the real gate, now done); the roles cluster staying in chat.js (status-adjacent, see detail above) is why it sits above 2500, and that is the intended end-state, not a shortfall. The one open item is the runtime `ReferenceError`-free load, folded into RM-6.
...
| NEW-FE-chatjs-split | P3 | **done (code)** — Tier-A 3 leaves + Tier-B 2 panels; chat.js 4254→2971; RM-6 runtime verify pending | Frontend |
