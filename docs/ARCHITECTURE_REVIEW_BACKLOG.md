...
**Status — scheduled into the RM-6 real-machine phase; autonomous cutover spec prepared (this run).** No JS model for channel messages: `grep 'let messages|messagesById|new Map(' chat.js` → 0 (the only `messages` symbol is the DOM container at the former `getElementById('messages')`). The read-cutover is **deliberately NOT done blind**: unlike the FE-2/FE-4 extractions (byte-exact moves whose behaviour is preserved by construction and verifiable by grep), FE-6 changes how values are *derived*, and several model-derived values must equal the previously-scraped rendered values — equivalence that only runtime can confirm. That is exactly why the finalized plan gates this whole item to the real-machine phase. The autonomous deliverable here is the turnkey cutover spec below (current line numbers, replacing the pre-Tier-B references).
