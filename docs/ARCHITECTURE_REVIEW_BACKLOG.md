...
| NEW-FE-chatjs-split | P3 | **done (code) — RM-6 runtime verify pending** | Tier-A `0bb67aa`/`4e74327`/`9c92cdd` (sounds/version-pill/help leaves). Tier-B `0a34a4b`/`e989cc1`/`2b3de87` — `_agentRoles`→Store, then settings.js (234 lines) + naming-lightbox.js (356) extracted as feature panels; the roles cluster (ROLE_PRESETS/_setRole/showBubbleRolePicker/fetchRoles) was kept in chat.js as status-adjacent (deviation from the "2 clean files" framing, same kind as FE-2's renderMarkdown/agentHats). chat.js 4254→2971. Static net: per-file `node --check` + 16-script concat check (no cross-script top-level redeclaration) + grep (0 moved defs left in chat.js). Coupled feature-panels → runtime verify folded into RM-6 |
