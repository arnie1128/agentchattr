@echo off
REM Start Claude wrapper for this project (auto-starts server if needed).
...
if not exist "%AGENTCHATTR_CONFIG_DIR%\_load.py" (
    echo ERROR: start_claude.cmd: %AGENTCHATTR_CONFIG_DIR%\_load.py not found. >&2
    endlocal
    exit /b 1
)
...
if "%AGENTCHATTR_ROOT%"=="" (
    echo ERROR: start_claude.cmd: _load.py did not produce AGENTCHATTR_ROOT. >&2
    endlocal
    exit /b 1
)

call "%AGENTCHATTR_ROOT%\launchers\windows\start_claude.bat" --agent-cwd "%AGENT_CWD%" %*
endlocal
