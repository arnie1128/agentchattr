...
When invoked through a project wrapper (`templates/project/start_<agent>.cmd`), the chain is:

```
PowerShell -> start_<agent>.cmd -> call windows/start_<agent>.bat
```
...
When adding `windows/start_<newagent>.bat`:

- [ ] Copy from `windows/start_claude.bat` as a baseline (clean post-fix template)
...
Get-ChildItem windows\*.bat | ForEach-Object {
...
Select-String -Path windows\*.bat -Pattern 'echo.*[()]' | Where-Object { $_.Line -notmatch '\^[()]' }
```
