...
# app_state, populated by app.configure(). Presence/activity/cursors/roles/
# last-read live on mcp_state; tools read them via mcp_state.X.
...
def _resolve_tool_identity(
    ...
    *,
    ...
    if inst:
        resolved = inst["name"]
        if resolved:
            mcp_state._touch_presence(resolved)
    ...
    if provided:
        mcp_state._touch_presence(provided)
...
def chat_read(
    ...
    if since_id:
        ...
        with mcp_state._cursors_lock:
            agent_cursors = mcp_state._cursors.get(sender, {})
    ...
    msgs = msgs[-limit:]
    mcp_state._update_cursor(sender, msgs, ch)
    serialized = _serialize_messages(msgs)

    # Escalating empty-read hints to discourage polling loops
    if not serialized and sender:
        mcp_state._empty_read_count[sender] = mcp_state._empty_read_count.get(sender, 0) + 1
        n = mcp_state._empty_read_count[sender]
        if n == 1:
        ...
        mcp_state._empty_read_count[sender] = 0
...
def chat_claim(sender: str, name: str = "", ctx: Context | None = None) -> str:
    ...
    mcp_state._touch_presence(confirmed)
