@echo off
REM Start Codex wrapper for this project (auto-starts server if needed).
...
if not exist "%AGENTCHATTR_CONFIG_DIR%\_load.py" (
    echo ERROR: start_codex.cmd: %AGENTCHATTR_CONFIG_DIR%\_load.py not found. >&2
    endlocal
    exit /b 1
)
...
if "%AGENTCHATTR_ROOT%"=="" (
    echo ERROR: start_codex.cmd: _load.py did not produce AGENTCHATTR_ROOT. >&2
    endlocal
    exit /b 1
)

call "%AGENTCHATTR_ROOT%\windows\start_codex.bat" --agent-cwd "%AGENT_CWD%" %*
endlocal
